...
@Component({
  ...
  `,
  styles: [`
    /* ESTILOS (mismo que antes) */
    .playlists-container { min-height: 100vh; background: linear-gradient(135deg, #0c0c0c 0%, #1a1a1a 100%); color: #e6eefc; padding-top: 80px; }
    .playlists-hero { height: 40vh; min-height: 300px; position: relative; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, rgba(139, 92, 246, 0.1) 0%, rgba(59, 130, 246, 0.08) 50%, rgba(6, 182, 212, 0.1) 100%); overflow: hidden; }
    .hero-content { text-align: center; z-index: 2; }
    .hero-title { font-size: 3.5rem; font-weight: 800; background: linear-gradient(135deg, #8b5cf6, #3b82f6, #06b6d4); background-clip: text; -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 1rem; }
    .hero-subtitle { font-size: 1.2rem; color: rgba(230, 238, 252, 0.8); margin-bottom: 2rem; font-weight: 300; }
    .create-playlist-btn { display: inline-flex; align-items: center; padding: 1rem 2rem; border-radius: 50px; border: none; color: #e6eefc; font-weight: 600; font-size: 1rem; cursor: pointer; transition: all 0.3s ease; }
    .create-playlist-btn:hover { transform: translateY(-3px); box-shadow: 0 15px 35px rgba(139, 92, 246, 0.3); }
    .w-5 { width: 1.25rem; height: 1.25rem; }
    .mr-2 { margin-right: 0.5rem; }
    .floating-elements { position: absolute; inset: 0; pointer-events: none; }
    .floating-note { position: absolute; font-size: 2rem; color: rgba(139, 92, 246, 0.3); animation: float 8s ease-in-out infinite; }
    ...
    .section-title { font-size: 2rem; font-weight: 700; background: linear-gradient(135deg, #e6eefc, #8b5cf6); background-clip: text; -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    .view-toggles { display: flex; gap: 0.5rem; }
    .view-btn { padding: 0.5rem; border-radius: 8px; border: none; background: rgba(255, 255, 255, 0.1); color: rgba(230, 238, 252, 0.7); cursor: pointer; transition: all 0.3s ease; }
    .view-btn:hover { background: rgba(255, 255, 255, 0.15); color: #e6eefc; }
    .view-btn.active { background: linear-gradient(135deg, #8b5cf6, #3b82f6); color: white; }
    .w-4 { width: 1rem; height: 1rem; }
    .empty-state { text-align: center; padding: 4rem 2rem; border-radius: 20px; }
    .empty-icon { width: 80px; height: 80px; margin: 0 auto 1.5rem; color: rgba(139, 92, 246, 0.5); }
    .empty-title { font-size: 1.5rem; font-weight: 600; color: #e6eefc; margin-bottom: 0.5rem; }
    .empty-text { color: rgba(230, 238, 252, 0.7); margin-bottom: 2rem; }
    .create-first-btn { display: inline-flex; align-items: center; gap: 0.5rem; padding: 0.75rem 1.5rem; border-radius: 25px; border: none; color: #e6eefc; font-weight: 600; cursor: pointer; transition: all 0.3s ease; }
    .create-first-btn:hover { transform: scale(1.05); background: linear-gradient(145deg, #8b5cf6, #7c3aed); }
    .loading-state { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 4rem 2rem; color: rgba(230, 238, 252, 0.7); }
    .spinner { width: 40px; height: 40px; border: 3px solid rgba(139, 92, 246, 0.2); border-top: 3px solid #8b5cf6; border-radius: 50%; animation: spin 1s linear infinite; margin-bottom: 1rem; }
    @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
    .playlists-content.grid-view { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 1.5rem; }
    .playlists-content.list-view { display: flex; flex-direction: column; gap: 1rem; }
    .playlist-card { padding: 1.5rem; border-radius: 20px; cursor: pointer; transition: all 0.3s ease; position: relative; }
    ...
    .playlist-card:hover { transform: translateY(-5px); box-shadow: 0 20px 40px rgba(139, 92, 246, 0.25); }
    .playlist-cover { position: relative; border-radius: 15px; overflow: hidden; margin-bottom: 1rem; }
    ...
    .playlist-overlay { position: absolute; inset: 0; background: rgba(0, 0, 0, 0.7); display: flex; align-items: center; justify-content: center; opacity: 0; transition: all 0.3s ease; }
    .playlist-card:hover .playlist-overlay { opacity: 1; }
    .play-all-btn { width: 50px; height: 50px; border-radius: 50%; border: none; color: #8b5cf6; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.3s ease; }
    .play-all-btn svg { width: 20px; height: 20px; margin-left: 2px; }
    .play-all-btn:hover { transform: scale(1.1); color: #7c3aed; }
    .privacy-badge { position: absolute; top: 8px; right: 8px; padding: 0.3rem; border-radius: 8px; display: flex; align-items: center; justify-content: center; }
    ...
    .playlist-name { font-size: 1.1rem; font-weight: 600; margin-bottom: 0.5rem; color: #e6eefc; }
    .playlist-description { color: rgba(230, 238, 252, 0.7); font-size: 0.9rem; margin-bottom: 1rem; line-height: 1.4; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
    ...
    .action-btn { padding: 0.5rem; border-radius: 8px; border: none; background: rgba(255, 255, 255, 0.1); color: rgba(230, 238, 252, 0.7); cursor: pointer; transition: all 0.3s ease; }
    ...
    .modal-overlay { position: fixed; inset: 0; background: rgba(0, 0, 0, 0.7); backdrop-filter: blur(5px); display: flex; align-items: center; justify-content: center; z-index: 1000; animation: fadeIn 0.3s ease; }
    @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
    .modal-content { width: 90%; max-width: 500px; border-radius: 20px; animation: slideUp 0.3s ease; }
    ...
    .close-btn { padding: 0.5rem; border-radius: 8px; border: none; background: rgba(255, 255, 255, 0.1); color: rgba(230, 238, 252, 0.7); cursor: pointer; transition: all 0.3s ease; }
    ...
    .form-input, .form-textarea { width: 100%; padding: 0.75rem; border-radius: 10px; border: none; color: #e6eefc; font-size: 1rem; outline: none; transition: all 0.3s ease; }
    .form-input:focus, .form-textarea:focus { box-shadow: 0 0 0 2px rgba(139, 92, 246, 0.5); }
    .form-textarea { resize: vertical; font-family: inherit; }
    .checkbox-label { display: flex; align-items: center; gap: 0.75rem; cursor: pointer; color: rgba(230, 238, 252, 0.9); }
    .form-checkbox { width: 18px; height: 18px; cursor: pointer; accent-color: #8b5cf6; }
    .modal-footer { display: flex; justify-content: flex-end; gap: 1rem; padding: 1.5rem; border-top: 1px solid rgba(255, 255, 255, 0.1); }
    .cancel-btn, .save-btn, .delete-confirm-btn { padding: 0.75rem 1.5rem; border-radius: 10px; border: none; font-weight: 600; cursor: pointer; transition: all 0.3s ease; }
    .cancel-btn { background: rgba(255, 255, 255, 0.1); color: #e6eefc; }
    .cancel-btn:hover { background: rgba(255, 255, 255, 0.15); }
    .save-btn { background: linear-gradient(135deg, #8b5cf6, #3b82f6); color: white; }
    ...
    .delete-confirm-btn { background: linear-gradient(135deg, #ef4444, #dc2626); color: white; }
    ...
    .glass-morphism { background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 0.1); box-shadow: 0 8px 32px rgba(0, 0, 0, 0.3); }
    .neomorphism { background: linear-gradient(145deg, #1a1a1a, #0f0f0f); box-shadow: 5px 5px 10px rgba(0, 0, 0, 0.5), -5px -5px 10px rgba(255, 255, 255, 0.02); border: 1px solid rgba(255, 255, 255, 0.05); }
    .fade-in-up { animation: fadeInUp 0.8s ease-out; animation-fill-mode: both; }
    @keyframes fadeInUp { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
    @media (max-width: 768px) { .playlists-container { padding-top: 70px; } .hero-title { font-size: 2.5rem; } .hero-subtitle { font-size: 1rem; } .main-content { padding: 1rem; } .section-header { flex-direction: column; gap: 1rem; align-items: flex-start; } .playlists-content.grid-view { grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); } }
    @media (max-width: 480px) { .playlists-container { padding-top: 65px; } .create-playlist-btn { padding: 0.8rem 1.5rem; font-size: 0.9rem; } .playlists-content.grid-view { grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 1rem; } .playlist-card { padding: 1rem; } .modal-content { width: 95%; } }
  `]
})
export class PlaylistsComponent implements OnInit, OnDestroy {
  ...
  // Navegar a la vista de detalle de la playlist
  goToPlaylistDetail(playlist: UserPlaylist) {
    this.router.navigate(['/playlist', playlist.id]);
  }

  async playPlaylist(playlist: UserPlaylist, event: Event) {
    event.stopPropagation();
    // Aquí puedes implementar la lógica para reproducir la playlist
