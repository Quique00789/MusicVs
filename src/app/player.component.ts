  ...
  template: `
    <div *ngIf="currentSong" class="fixed bottom-4 left-4 right-4 z-50 bg-gradient-to-t from-slate-900/95 to-black/80 backdrop-blur-lg border border-white/5 rounded-2xl p-4 relative">
      ...
      <div class="hidden sm:block">
        <div class="max-w-7xl mx-auto flex items-center justify-between gap-4">
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="relative">
              <button (click)="toggleQueue()" class="p-2 text-gray-300 hover:text-white transition-all duration-200 group relative hover:scale-110" [class.text-cyan-400]="showQueue()" title="Cola de reproducción">
      ...
      <div class="block sm:hidden space-y-3">
        <!-- Fila 1: Imagen con visualizador + Info de la canción + Botones principales -->
        <div class="flex items-center gap-3">
        ...
        <!-- Fila 2: Barra de progreso con tiempo -->
        <div class="flex items-center gap-3">
        ...
        <!-- Fila 3: Controles adicionales (volumen, velocidad, cola) -->
        <div class="flex items-center justify-between gap-2">
          ...
          <!-- Botón de cola -->
          <button (click)="toggleQueue()" class="p-1.5 text-gray-300 hover:text-white transition-all duration-200" [class.text-cyan-400]="showQueue()" title="Cola">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M4 6h16M4 10h16M4 14h16M4 18h16"/>
            </svg>
          </button>
...
export class PlayerComponent implements AfterViewInit {
  ...
  // Estado de mute local para UI y para recordar el volumen anterior
  isMuted = false;
  ...
  private startAnimations() {
    const animate = () => {
      this.visualizerBars = this.isPlaying
        ? this.visualizerBars.map(() => Math.random() * 70 + 10)
        : this.visualizerBars.map(bar => Math.max(10, bar * 0.95));
  ...
  onToggleMuteClick() {
    if (!this.isMuted) {
      ...
      this.isMuted = false;
      this.setVolume.emit(this.previousVolume);
    }
  }
